...
allUnits.plantSpirit={name:'🌳',cost:14};
allUnits.digger={name:'⛏️',cost:8};
...
const UNITES_A_DEBLOQUER = ['mage','trebuchet','electromancer','cryomancer',
  'pyromancer','mole','cavalier','necromancer','bannerman','lizard','plantSpirit','digger',
...
const rarities={
 ...
 skeletonCard:['🔵','Rare'],
 skeletonGeneral:['🟠','Légendaire'], plantSpirit:['🟠','Légendaire'], digger:['🟣','Épique'],
...
// Le poids dépend du TYPE de carte, jamais de ses PV du moment : un
// chevalier reste léger qu'il soit niveau 1 ou niveau 20, ses PV montent
// mais son gabarit ne change pas. Seules les cartes listées ici sont trop
// lourdes pour être arrachées du sol, indépendamment du niveau.
...
// ---------- RÈGLES DU PÊCHEUR À LA PELLE ----------
const PELLE_PORTEE_TIR   = 230;  // jusqu'où il projette ses débris
const PELLE_CREUSE       = 40;   // images passées à creuser avant de lancer
const PELLE_EN_LAIR      = 22;   // le temps que la motte reste en l'air
const PELLE_RECHARGE     = 55;   // repos avant de recreuser
const PELLE_NB_DEBRIS    = 5;    // petits cailloux projetés
const PELLE_DEBRIS_DMG   = 0.30; // part des dégâts pour chaque petit débris
const PELLE_GROS_DMG     = 0.85; // le gros caillou, bien plus douloureux

// ---------- RÈGLES DE L'ESPRIT DES PLANTES ET DE L'ARBRE ANCESTRAL ----------
const ARBRE_RAYON_SOIN = 160;   // large : c'est un arbre de vie, pas un guérisseur discret
const RACINES_SECONDAIRE = 0.15; // part des dégâts par tick : petits coups répétés, pas un gros impact
const LIGNE_RACINES_LONGUEUR = 130;  // portée de la ligne de racines au sol
const LIGNE_RACINES_DUREE    = 220;  // environ 3,7 secondes avant que la ligne disparaisse
const LIGNE_RACINES_POUSSE   = 55;   // les racines mettent presque 1 seconde à sortir de terre
const LIGNE_RACINES_TICK     = 9;    // dégâts fréquents, mais chacun tout petit
// Trop massifs pour être tractés, quels que soient leurs PV du moment.
const TROP_LOURDS = ['mole','giant','trebuchet','hauntedCarriage',
                     'skeletonGeneral','guardian','plantSpirit'];
...
const DMG_BATIMENT={
 ...
 necromancer:8, bannerman:8, skeleton:3, lizard:10, plantSpirit:18, ancientTree:0, digger:11,
...
// --- ESPRIT DES PLANTES --- colosse de bois et de racines. Fait jaillir
// des pointes autour de ses cibles, et laisse pousser un arbre à sa mort.
plantSpirit:{hp:650,dmg:24,speed:0.55,range:42,color:'#4a7a3a'},

// --- PÊCHEUR À LA PELLE --- il creuse le sol, lance la terre en l'air et
// la frappe pour projeter une volée de débris. Au contact, coups tranchants.
digger:{hp:260,dmg:18,speed:1.05,range:40,color:'#d8b43a'},

// L'arbre ancestral qui pousse à la mort de l'esprit. Ne se bat jamais,
// perd un peu de vie avec le temps, régénère les alliés proches, et peut
// être détruit par l'ennemi comme n'importe quelle unité.
ancientTree:{hp:800,dmg:0,speed:0,range:0,color:'#6b4a2e'},
...
const BODY={
  ...
  plantSpirit:{r:27,m:12}, ancientTree:{r:24,m:14}, digger:{r:19,m:3.2}
}[type] || {r:20,m:3};
this.radius=BODY.r;
this.mass=BODY.m;
...
// === PÊCHEUR À LA PELLE : creuse, lance, frappe ===
// Trois temps. Il plante sa pelle et creuse, envoie la motte en l'air,
// puis la frappe en plein vol pour projeter une volée de débris vers
// l'ennemi. Au corps à corps, il donne des coups de pelle tranchants.
if(this.type==='digger'){
  if(this.pPhase===undefined){ this.pPhase='marche'; this.pT=0; this.pRecharge=0; }
  if(this.pRecharge>0) this.pRecharge--;

  // Un ennemi au contact : il se bat à la pelle, il ne creuse pas.
  let contact=false, loin=null, dLoin=1e9;
  for(let i=0;i<unitsArr.length;i++){
    const c=unitsArr[i];
    ...
    if(d < this.range+this.radius+(c.radius||20)+6) contact=true;
    if(d<PELLE_PORTEE_TIR && d<dLoin){ dLoin=d; loin=c; }
  }

  if(contact){
    // Il abandonne son trou et se défend : coups de pelle normaux.
    this.pPhase='marche'; this.pT=0;
  }
  else if(this.pPhase==='marche' && loin && this.pRecharge<=0){
    this.pPhase='creuse'; this.pT=0; this.pCible=loin;
  }
  else if(this.pPhase==='creuse'){
    this.pT++;
    if(this.pT>=PELLE_CREUSE){ this.pPhase='enlair'; this.pT=0; }
  }
  else if(this.pPhase==='enlair'){
    this.pT++;
    if(this.pT>=PELLE_EN_LAIR){
      // --- LE COUP DE PELLE : la motte explose en débris ---
      const cible=(this.pCible && this.pCible.hp>0) ? this.pCible : loin;
      if(cible){
        // Le bonus du porte-étendard est recalculé ici : la constante
        // globale RALLY_DMG est déclarée plus bas dans la méthode, elle
        // n'existe pas encore à ce stade.
        const bonusRallye = this.rallyTime>0 ? 1.25 : 1;
        // Le gros caillou part droit sur la cible visée.
        projectiles.push({x:this.x,y:this.y-26,target:cible,
          dmg:this.dmg*bonusRallye*PELLE_GROS_DMG,
          sourceType:'digger', grosCaillou:true});
        // Les petits débris arrosent tout ce qui est proche d'elle.
        let envoyes=0;
        for(let k=0;k<unitsArr.length && envoyes<PELLE_NB_DEBRIS;k++){
          const v=unitsArr[k];
          if(v.team===this.team||v.hp<=0||v.lane!==this.lane) continue;
          if(Math.hypot(v.x-cible.x,(v.y-cible.y)*0.8)>90) continue;
          projectiles.push({x:this.x,y:this.y-26,target:v,
            dmg:this.dmg*bonusRallye*PELLE_DEBRIS_DMG,
            sourceType:'digger'});
          envoyes++;
        }
      }
      this.frappePelle=14;      // animation du grand coup de pelle
      this.pPhase='marche'; this.pT=0; this.pRecharge=PELLE_RECHARGE;
    }
  }
  if(this.frappePelle>0) this.frappePelle--;
}

// === ESPRIT DES PLANTES : posture changeante, régénération légère ===
// Il se redresse sur deux pattes quand il combat, sinon il avance à quatre
// pattes comme un vrai golem animal. Il suinte un peu d'eau en continu.
if(this.type==='plantSpirit'){
  this.deuxPattes = this._attacking||false;
  // Il se régénère très lentement lui-même, comme une plante qui pousse.
  if(this.hp>0 && this.hp<this.maxHp && (this.animTick||0)%40===0) this.hp=Math.min(this.maxHp,this.hp+1);
}

// === ARBRE ANCESTRAL : pousse à la mort de l'Esprit des Plantes ===
// Il ne combat jamais, régénère les alliés autour de lui, et s'éteint
// doucement avec le temps — mais l'ennemi peut aussi l'abattre avant ça.
if(this.type==='ancientTree'){
  if(this.pousse===undefined){ this.pousse=30; this.tickVie=0; }
  if(this.pousse>0) this.pousse--;         // anime sa croissance à l'arrivée

  // Il s'éteint lentement : environ 1% de sa vie max toutes les secondes.
  this.tickVie++;
  if(this.tickVie>=60){
    this.tickVie=0;
    if(this.hp>0) this.hp -= Math.max(1, Math.round(this.maxHp*0.01));
  }

  // Régénère tous les alliés dans un large rayon, pas seulement les
  // squelettes : c'est un arbre de vie, pas une relique nécromantique.
  if(this.hp>0){
    for(let i=0;i<unitsArr.length;i++){
      const u=unitsArr[i];
      if(u.team!==this.team||u.hp<=0||u.hp>=u.maxHp||u===this) continue;
      if(Math.hypot(u.x-this.x,(u.y-this.y)*0.8)<ARBRE_RAYON_SOIN){
        u.hp=Math.min(u.maxHp,u.hp+0.22);
        if(Math.random()<0.03) u.soinFlash=12;
      }
    }
  }
}
...
if(this.type==='lizard'){
  ...
  if(this.phase==='chasse'){
    ...
    for(let i=0;i<unitsArr.length;i++){
      ...
      if(c.team===this.team||c.hp<=0||c.lane!==this.lane) continue;
      if(c.type==='mole'&&c.burrow>0) continue;
      // Seul le TYPE de carte détermine si elle est trop lourde. Les PV
      // ne rentrent plus en compte : ils montent avec le niveau, mais ça
      // ne change rien au gabarit physique de l'unité.
...
if(target){
 if(this.cool<=0){
  ...
  else if(this.type==='plantSpirit'){
   // Il frappe le sol : ça inflige les dégâts normaux à sa cible, ET ça
   // fait apparaître une LIGNE de racines pointues devant lui — mais
   // SEULEMENT s'il n'en a pas déjà une active. Sinon, comme il frappe
   // bien plus vite que le temps de pousse des racines, chaque coup
   // remplaçait l'ancienne ligne avant qu'elle ait fini de sortir de
   // terre, et elle ne faisait alors jamais aucun dégât.
   target.hp-=this.dmg*RALLY_DMG;
   const dejaActive = this._zoneRacine && racineZones.includes(this._zoneRacine);
   if(!dejaActive){
     const sens=this.team==='player'?1:-1;
     this._zoneRacine={
       x:this.x, y:this.y, team:this.team, lane:this.lane,
       longueur:LIGNE_RACINES_LONGUEUR, sens,
       dmg:this.dmg*RALLY_DMG*RACINES_SECONDAIRE,
       vie:LIGNE_RACINES_DUREE, tick:0
     };
     racineZones.push(this._zoneRacine);
   }
   this.racinesFlash=16;   // animation des pointes qui sortent du sol
  }
 ...
 if(this.type!=='healer'){
   ...
   // Le pêcheur plante sa pelle : il ne marche pas pendant qu'il creuse
   // et qu'il envoie sa motte en l'air.
   const pecheurAuTrou = (this.type==='digger' && (this.pPhase==='creuse'||this.pPhase==='enlair'));
   const moveSpeed=(this.harponne>0||lezardEnPosition||pecheurAuTrou) ? 0
...
if(t==='necromancer'||t==='bannerman'||t==='skeleton'||t==='skeletonCard'
   ...
   ||t==='plantSpirit'||t==='ancientTree'||t==='digger'){
  ctx.save();
  const bob=Math.sin(tk*0.12)*2;
  const yy=y+bob;
  if(t==='digger'){
    // --- PÊCHEUR À LA PELLE ---
    const creuse = this.pPhase==='creuse';
    const enlair = this.pPhase==='enlair';
    const frappe = this.frappePelle>0;
    const pas=Math.sin(tk*0.2);
    const peau='#d8a878', tunic='#5a7a4a', jaune='#e8c33a', jauneF='#a8871f';

    ctx.fillStyle='rgba(0,0,0,0.22)';
    ctx.beginPath(); ctx.ellipse(x,yy+23,13,4,0,0,6.28); ctx.fill();

    // Le trou qu'il creuse, avec le tas de terre à côté
    if(creuse||enlair){
      ctx.fillStyle='#4a3a26';
      ctx.beginPath(); ctx.ellipse(x+16*dir,yy+21,13,5,0,0,6.28); ctx.fill();
      ctx.fillStyle='#6b5535';
      ctx.beginPath(); ctx.ellipse(x+30*dir,yy+20,8,4,0,0,6.28); ctx.fill();
    }

    // Jambes
    ctx.strokeStyle='#3a4a2e'; ctx.lineWidth=4; ctx.lineCap='round';
    const ecart = (creuse||enlair) ? 5 : pas*4;
    ctx.beginPath(); ctx.moveTo(x,yy+8); ctx.lineTo(x-4-ecart,yy+21); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(x,yy+8); ctx.lineTo(x+4+ecart,yy+21); ctx.stroke();

    // Torse, penché en avant quand il creuse
    const penche = creuse ? 0.3 : 0;
    ctx.save();
    ctx.translate(x,yy);
    ctx.rotate(penche*dir);
    ctx.fillStyle=tunic;
    ctx.beginPath();
    ctx.moveTo(-9,-12); ctx.lineTo(9,-12); ctx.lineTo(7,9); ctx.lineTo(-7,9);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle='#8a6a3a';
    ctx.fillRect(-8,-1,16,3);                    // ceinture

    // Tête, chapeau de paille
    ctx.fillStyle=peau;
    ctx.beginPath(); ctx.arc(0,-19,7,0,6.28); ctx.fill();
    ctx.fillStyle='#c8a860';
    ctx.beginPath(); ctx.ellipse(0,-24,12,3.5,0,0,6.28); ctx.fill();
    ctx.beginPath(); ctx.arc(0,-25,6,Math.PI,0); ctx.fill();
    ctx.fillStyle='#2e2820';
    ctx.fillRect(2*dir,-20,2,2);
    ctx.restore();

    // --- LA PELLE, en métal jaune ---
    // Elle change de position selon ce qu'il fait : plantée dans le sol,
    // levée en garde, ou en plein grand coup horizontal.
    let angle, mx, my;
    if(frappe){ angle=-1.5*dir; mx=x+6*dir; my=yy-16; }        // grand coup
    else if(creuse){ angle=0.9*dir; mx=x+9*dir; my=yy-2; }      // plantée au sol
    else if(enlair){ angle=-0.5*dir; mx=x+9*dir; my=yy-10; }    // levée, prête
    else { angle=(attacking?-0.9:0.35)*dir; mx=x+8*dir; my=yy-6; }

    ctx.save();
    ctx.translate(mx,my);
    ctx.rotate(angle);
    ctx.strokeStyle='#8a6a42'; ctx.lineWidth=3.5; ctx.lineCap='round';
    ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(0,20); ctx.stroke();   // manche
    ctx.fillStyle=jaune;                                               // fer jaune
    ctx.beginPath();
    ctx.moveTo(-7,20); ctx.lineTo(7,20); ctx.lineTo(5,33); ctx.lineTo(-5,33);
    ctx.closePath(); ctx.fill();
    ctx.strokeStyle=jauneF; ctx.lineWidth=1.5;
    ctx.beginPath(); ctx.moveTo(0,21); ctx.lineTo(0,32); ctx.stroke();
    ctx.fillStyle='#fff6c0';                                           // tranchant
    ctx.fillRect(-5,31,10,2);
    ctx.restore();

    // Bras tendus vers le manche
    ctx.strokeStyle=peau; ctx.lineWidth=3;
    ctx.beginPath(); ctx.moveTo(x+3*dir,yy-8); ctx.lineTo(mx,my+4); ctx.stroke();

    // --- LA MOTTE EN L'AIR ---
    if(enlair){
      const p=this.pT/PELLE_EN_LAIR;
      const h=Math.sin(p*Math.PI)*44;             // trajectoire en cloche
      const mxx=x+14*dir, myy=yy-6-h;
      ctx.fillStyle='#6b5535';
      ctx.beginPath(); ctx.arc(mxx,myy,9,0,6.28); ctx.fill();
      ctx.fillStyle='#8a7550';
      ctx.beginPath(); ctx.arc(mxx-3,myy-3,4,0,6.28); ctx.fill();
      ctx.fillStyle='#5a5348';                     // cailloux dans la motte
      ctx.fillRect(mxx+2,myy+1,3,3);
      ctx.fillRect(mxx-5,myy+3,2,2);
      // Terre qui s'effrite en montant
      for(let d=0;d<3;d++){
        ctx.fillStyle='rgba(107,85,53,0.5)';
        ctx.fillRect(mxx-8+Math.random()*16, myy+8+Math.random()*10, 2,2);
      }
    }

    // Pelletées de terre pendant qu'il creuse
    if(creuse && this.pT%7===0){
      for(let d=0;d<3;d++){
        ctx.fillStyle='rgba(107,85,53,0.6)';
        ctx.fillRect(x+18*dir+Math.random()*10, yy+4+Math.random()*14, 3,3);
      }
    }

    // Éclat au moment du grand coup
    if(frappe){
      ctx.fillStyle='rgba(255,246,192,'+(this.frappePelle/14*0.5).toFixed(2)+')';
      ctx.beginPath(); ctx.arc(x+22*dir,yy-14,14,0,6.28); ctx.fill();
    }
    ctx.restore();
    return;
  }
  ...
  if(t==='plantSpirit'){
    // --- ESPRIT DES PLANTES ---
    const debout=this.deuxPattes;
    const pas=Math.sin(tk*0.15);
    const bois='#5a4530', boisF='#3d2e1f', feuille='#4a7a3a';

    ctx.fillStyle='rgba(0,0,0,0.26)';
    ctx.beginPath(); ctx.ellipse(x,yy+28,22,6,0,0,6.28); ctx.fill();

    if(!debout){
      ctx.strokeStyle=boisF; ctx.lineWidth=7; ctx.lineCap='round';
      [[-14,1],[14,1],[-9,-1],[9,-1]].forEach((p,i)=>{
        const ph=pas*(i%2?1:-1);
        ctx.beginPath();
        ctx.moveTo(x+p[0],yy+4);
        ctx.lineTo(x+p[0]+ph*5,yy+22);
        ctx.stroke();
      });
    } else {
      ctx.strokeStyle=boisF; ctx.lineWidth=9; ctx.lineCap='round';
      ctx.beginPath(); ctx.moveTo(x-8,yy+6); ctx.lineTo(x-10,yy+26); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(x+8,yy+6); ctx.lineTo(x+10,yy+26); ctx.stroke();
    }

    ctx.fillStyle=bois;
    ctx.beginPath(); ctx.ellipse(x,yy-6,20,17,0,0,6.28); ctx.fill();
    ctx.strokeStyle=boisF; ctx.lineWidth=2;
    for(let e=-1;e<2;e++){
      ctx.beginPath();
      ctx.moveTo(x+e*10,yy-20); ctx.quadraticCurveTo(x+e*13,yy-6,x+e*9,yy+8);
      ctx.stroke();
    }
    if(Math.sin(tk*0.05)>0.3){
      ctx.strokeStyle='rgba(140,200,230,0.55)'; ctx.lineWidth=2;
      ctx.beginPath(); ctx.moveTo(x-6,yy-2); ctx.lineTo(x-7,yy+16); ctx.stroke();
    }
    ctx.fillStyle=feuille;
    [[-16,-16],[16,-16],[-12,-22],[12,-22]].forEach(p=>{
      ctx.beginPath();
      ctx.ellipse(x+p[0],yy+p[1],5,3,Math.sign(p[0])*0.6,0,6.28);
      ctx.fill();
    });

    const griffe=this._attacking;
    ctx.strokeStyle=bois; ctx.lineWidth=6;
    ctx.beginPath();
    ctx.moveTo(x+16*dir,yy-8);
    ctx.lineTo((x+(griffe?30:24)*dir),yy+(griffe?-14:2));
    ctx.stroke();

    ctx.fillStyle='#7a6042';
    ctx.beginPath(); ctx.ellipse(x,yy-24,13,11,0,0,6.28); ctx.fill();
    ctx.strokeStyle=boisF; ctx.lineWidth=1.4;
    ctx.beginPath(); ctx.moveTo(x-9,yy-24); ctx.lineTo(x+9,yy-24); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(x,yy-33); ctx.lineTo(x,yy-15); ctx.stroke();
    ctx.beginPath(); ctx.arc(x,yy-24,7,0.3,2.8); ctx.stroke();
    const lueur=0.6+0.4*Math.sin(tk*0.12);
    ctx.fillStyle='rgba(140,230,140,'+lueur.toFixed(2)+')';
    ctx.beginPath(); ctx.arc(x-4.5,yy-25,2.4,0,6.28); ctx.fill();
    ctx.beginPath(); ctx.arc(x+4.5,yy-25,2.4,0,6.28); ctx.fill();
    ctx.strokeStyle=boisF; ctx.lineWidth=2.5;
    ctx.beginPath(); ctx.moveTo(x-5,yy-33); ctx.lineTo(x-8,yy-42); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(x+5,yy-33); ctx.lineTo(x+8,yy-42); ctx.stroke();

    if(this.racinesFlash>0){
      const p2=this.racinesFlash/16;
      for(let r=0;r<5;r++){
        const ang=r*1.25;
        const rx=x+40*dir+Math.cos(ang)*22, ry=yy+10+Math.sin(ang)*10;
        ctx.strokeStyle='#8a7550'; ctx.lineWidth=3;
        ctx.beginPath();
        ctx.moveTo(rx,ry+14*p2);
        ctx.lineTo(rx,ry+14*p2-14*p2);
        ctx.stroke();
      }
      this.racinesFlash--;
    }
  ...
  if(t==='ancientTree'){
    const g=Math.min(1,1-(this.pousse||0)/30);
    ctx.globalAlpha=0.3+0.7*g;
    const h=g;

    ctx.fillStyle='rgba(0,0,0,0.25)';
    ctx.beginPath(); ctx.ellipse(x,yy+26,20*h,6,0,0,6.28); ctx.fill();

    ctx.fillStyle='#6b4a2e';
    ctx.beginPath();
    ctx.moveTo(x-8*h,yy+24); ctx.lineTo(x-5*h,yy-20*h);
    ctx.lineTo(x+5*h,yy-20*h); ctx.lineTo(x+8*h,yy+24);
    ctx.closePath(); ctx.fill();
    ctx.strokeStyle='#4a3320'; ctx.lineWidth=1.4;
    ctx.beginPath(); ctx.moveTo(x-2,yy+18); ctx.quadraticCurveTo(x+2,yy,x-1,yy-16*h); ctx.stroke();

    ctx.strokeStyle='#4a3320'; ctx.lineWidth=3;
    [-12,-4,4,12].forEach(o=>{
      ctx.beginPath(); ctx.moveTo(x,yy+22); ctx.lineTo(x+o*h,yy+30); ctx.stroke();
    });

    const sante=this.hp/this.maxHp;
    const vert = sante>0.4 ? '#4a8a3a' : '#8a7a3a';
    ctx.fillStyle=vert;
    ctx.beginPath(); ctx.arc(x,yy-30*h,20*h+6,0,6.28); ctx.fill();
    ctx.fillStyle = sante>0.4 ? '#5aa04a' : '#a08a4a';
    ctx.beginPath(); ctx.arc(x-6*h,yy-36*h,10*h+3,0,6.28); ctx.fill();
    ctx.beginPath(); ctx.arc(x+7*h,yy-34*h,9*h+3,0,6.28); ctx.fill();

    if(this.hp>0 && h>0.5){
      const puls=ARBRE_RAYON_SOIN*0.15+Math.sin(tk*0.08)*8;
      ctx.strokeStyle='rgba(140,230,140,0.18)'; ctx.lineWidth=2;
      ctx.beginPath(); ctx.arc(x,yy,puls,0,6.28); ctx.stroke();
    }
    if(this.soinFlash>0){
      ctx.fillStyle='rgba(180,255,180,0.85)';
      ctx.fillRect(x-3+Math.random()*10,yy-40*h-Math.random()*10,3,3);
      this.soinFlash--;
    }
    ctx.globalAlpha=1;
...
const INFOS_UNITES = {
 ...
 digger:      {nom:'Pêcheur à la Pelle', pouvoir:'Sa pelle de métal jaune est autant un outil qu\'une arme. À distance, il CREUSE le sol, envoie la motte en l\'air et la FRAPPE en plein vol : un gros caillou part sur sa cible et une volée de petits débris arrose ceux qui sont autour. Dès qu\'un ennemi arrive au contact, il abandonne son trou et se défend à grands coups de pelle tranchants.'},
 plantSpirit: {nom:'Esprit des Plantes', pouvoir:'Colosse de bois et de racines. Une LIGNE DE RACINES met presque 1 seconde à sortir de terre devant lui, puis blesse un peu à chaque instant tout ennemi qui reste dedans, pendant plusieurs secondes. À sa mort, un ARBRE ANCESTRAL très résistant pousse à sa place : il régénère tous les alliés dans une large zone. Il ne se bat jamais, perd un peu de vie avec le temps, et peut être détruit par l\'ennemi.'},
 ancientTree: {nom:'Arbre Ancestral', pouvoir:'Ne combat jamais. Régénère les alliés autour de lui et s\'éteint lentement. Sort de l\'Esprit des Plantes à sa mort. Ne s\'achète pas.'},
 lizard:      {nom:'Lézard à Casque', pouvoir:'Projette sa LANGUE jusqu\'à 265 pour enrouler un ennemi et le ramener d\'un coup sur la pointe de son casque : 34 dégâts d\'empalement, puis il l\'achève à coups de griffes. Sa proie ne peut plus bouger pendant la traction. Les colosses (gardiens, géants, taupes, calèches, trébuchets...) sont trop lourds pour être arrachés du sol, quel que soit leur niveau.'},
...
const unitMeta={
 ...
 plantSpirit:{rarity:'legendary',role:'tank'},
 digger:{rarity:'epic',role:'melee'},
