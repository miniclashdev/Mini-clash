...
const LANGUE_TRACTION = 3.4;   // vitesse à laquelle la proie est ramenée
const LANGUE_RECHARGE = 70;    // repos entre deux harponnages
const LANGUE_POIDS_MAX= 420;   // au-delà, la cible est trop lourde
const DEGATS_POINTE   = 34;    // dégâts de l'empalement sur le casque
const GRIFFES_DUREE   = 84;    // durée de la série de coups de griffes
...
if(this.type==='lizard'){
  ...
  // --- 1. CHASSE : il repère une proie à portée de langue ---
  if(this.phase==='chasse' && this.recharge<=0){
    let proie=null, best=1e9;
    for(let i=0;i<unitsArr.length;i++){
      const c=unitsArr[i];
      if(c.team===this.team||c.hp<=0||c.lane!==this.lane) continue;
      if(c.type==='mole'&&c.burrow>0) continue;
      // Les colosses ne s'arrachent pas du sol : on écarte les grosses
      // barres de vie, et nommément les mastodontes dont les PV varient
      // en cours de partie.
      if(c.maxHp>LANGUE_POIDS_MAX) continue;
      if(TROP_LOURDS.includes(c.type)) continue;
      const d=Math.hypot(c.x-this.x,(c.y-this.y)*0.8);
      // Pas les cibles déjà au contact : la langue sert à aller CHERCHER loin
      if(d>50 && d<LANGUE_PORTEE && d<best){ best=d; proie=c; }
    }
    if(proie){
      this.phase='langue'; this.langueT=0; this.proie=proie;
      // Durée d'aller proportionnelle : une cible lointaine met plus
      // longtemps à être atteinte, on voit la langue traverser le terrain.
...
if(target){
 ...
 if(this.type!=='healer'){
   ...
   const moveSpeed=this.harponne>0 ? 0
...
if(t==='necromancer'||t==='bannerman'||t==='skeleton'||t==='skeletonCard'
  ...
  if(t==='lizard'){
    // --- LÉZARD À CASQUE ---
    // Bipède penché en avant, queue en contrepoids, casque à longue pointe.
    const pas=Math.sin(tk*0.19);
    const penche=this.teteBaissee>0 ? 0.42 : 0.22;   // il baisse la tête à l'impact
    const vert='#5f9a4a', vertF='#48783a', ventre='#c8d68a';

    ctx.fillStyle='rgba(0,0,0,0.24)';
    ctx.beginPath(); ctx.ellipse(x,yy+24,15,5,0,0,6.28); ctx.fill();

    // Queue épaisse qui fouette pour l'équilibre
    ctx.strokeStyle=vertF; ctx.lineWidth=7; ctx.lineCap='round';
    ctx.beginPath();
    ctx.moveTo(x-9*dir,yy+8);
    ctx.quadraticCurveTo(x-26*dir,yy+10+pas*5,x-38*dir,yy-2+pas*8);
    ctx.stroke();
    ctx.strokeStyle=vert; ctx.lineWidth=3;
    ctx.beginPath();
    ctx.moveTo(x-9*dir,yy+8);
    ctx.quadraticCurveTo(x-26*dir,yy+10+pas*5,x-36*dir,yy-1+pas*8);
    ctx.stroke();

    // Pattes arrière repliées, démarche de bipède
    ctx.strokeStyle=vertF; ctx.lineWidth=5;
    [[-1,1],[1,-1]].forEach((s,i)=>{
      const ph=pas*s[0];
      ctx.beginPath();
      ctx.moveTo(x-2*dir,yy+9);
      ctx.lineTo(x-7*dir+ph*4,yy+17);
      ctx.lineTo(x+2*dir+ph*6,yy+24);
      ctx.stroke();
    });
    ...
    ctx.save();
    ctx.translate(x,yy);
    ctx.rotate(-penche*0.45*dir);
    ctx.fillStyle=vert;
    ctx.beginPath(); ctx.ellipse(0,-2,15,11,0,0,6.28); ctx.fill();
    ctx.fillStyle=ventre;                          // ventre plus clair
    ctx.beginPath(); ctx.ellipse(1,3,11,6,0,0,6.28); ctx.fill();
    // Écailles dorsales
    ctx.fillStyle=vertF;
    for(let e=0;e<4;e++){
      ctx.beginPath();
      ctx.moveTo(-9+e*6,-10); ctx.lineTo(-6+e*6,-15); ctx.lineTo(-3+e*6,-10);
      ctx.closePath(); ctx.fill();
    }
    // Bras griffus
    const griffe=this.griffeFlash>0 ? 1 : 0;
    ctx.strokeStyle=vert; ctx.lineWidth=4;
    ctx.beginPath();
    ctx.moveTo(6*dir,0);
    ctx.lineTo((14+griffe*5)*dir,(griffe?-6:4));
    ctx.stroke();
    ctx.strokeStyle='#e8e0c8'; ctx.lineWidth=1.6;   // les griffes
    for(let g=0;g<3;g++){
      ctx.beginPath();
      ctx.moveTo((14+griffe*5)*dir,(griffe?-6:4));
      ctx.lineTo((20+griffe*7)*dir,(griffe?-11:8)+g*3);
      ctx.stroke();
    }
    ctx.restore();

    // --- Tête et casque, inclinés vers l'avant ---
    ctx.save();
    ctx.translate(x+9*dir,yy-16);
    ctx.rotate(penche*dir);
    ctx.fillStyle=vert;
    ctx.beginPath(); ctx.ellipse(0,0,11,8,0,0,6.28); ctx.fill();
    ctx.fillStyle=ventre;
    ctx.beginPath(); ctx.ellipse(3*dir,3,7,4,0,0,6.28); ctx.fill();
    // Œil reptilien à pupille fendue
    ctx.fillStyle='#f0d060';
    ctx.beginPath(); ctx.arc(4*dir,-2,3.2,0,6.28); ctx.fill();
    ctx.fillStyle='#1a1a1a';
    ctx.fillRect(3.2*dir,-4.4,1.6,4.8);
    // Casque de fer
    ctx.fillStyle='#8d959e';
    ctx.beginPath(); ctx.arc(0,-3,11,Math.PI,0); ctx.fill();
    ctx.fillStyle='#6b737c';
    ctx.fillRect(-11,-4,22,3);
    // LA POINTE : longue, effilée, c'est son arme
    ctx.fillStyle='#c8ccd2';
    ctx.beginPath();
    ctx.moveTo(2*dir,-12);
    ctx.lineTo(26*dir,-19);
    ctx.lineTo(4*dir,-6);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle='#eef2f6';
    ctx.beginPath();
    ctx.moveTo(3*dir,-11); ctx.lineTo(24*dir,-18); ctx.lineTo(4*dir,-9);
    ctx.closePath(); ctx.fill();
    ctx.restore();

    // --- LANGUE AU REPOS : elle pendouille et se balance ---
    // Un lézard garde la langue sortie ; ça donne vie au personnage et on
    // comprend tout de suite d'où part le harpon.
    if(this.phase!=='langue'){
      const bx=x+12*dir, by=yy-14;
      const bal=Math.sin(tk*0.13);
      const rec=this.recharge>0 ? 0.5 : 1;   // rentrée pendant la recharge
      ctx.strokeStyle='#e0607a'; ctx.lineWidth=3.5; ctx.lineCap='round';
      ctx.beginPath();
      ctx.moveTo(bx,by);
      ctx.quadraticCurveTo(bx+(9*rec)*dir, by+9*rec,
                           bx+(14*rec)*dir+bal*4, by+(17*rec)+bal*2);
      ctx.stroke();
      ctx.strokeStyle='#f090a4'; ctx.lineWidth=1.6;
      ctx.beginPath();
      ctx.moveTo(bx,by);
      ctx.quadraticCurveTo(bx+(9*rec)*dir, by+9*rec,
                           bx+(13*rec)*dir+bal*4, by+(16*rec)+bal*2);
      ctx.stroke();
      // Pointe fourchue au bout
      ctx.strokeStyle='#e0607a'; ctx.lineWidth=2;
      ctx.beginPath();
      ctx.moveTo(bx+(14*rec)*dir+bal*4, by+(17*rec)+bal*2);
      ctx.lineTo(bx+(18*rec)*dir+bal*4, by+(20*rec)+bal*2);
      ctx.stroke();
    }

    // --- LA LANGUE EN ACTION ---
    if(this.phase==='langue' && this.proie){
      const p=this.proie;
      const bx=x+12*dir, by=yy-14;
      const t2=Math.min(1,this.langueT/(this.langueAller||LANGUE_ALLER_MIN));
      const ex=bx+(p.x-bx)*t2, ey=by+(p.y-by)*t2;
      // Elle ondule pendant qu'elle se déroule
      ctx.strokeStyle='#e0607a'; ctx.lineWidth=5.5; ctx.lineCap='round';
      ctx.beginPath();
      ctx.moveTo(bx,by);
      ctx.quadraticCurveTo((bx+ex)/2,(by+ey)/2+Math.sin(tk*0.5)*9,ex,ey);
      ctx.stroke();
      ctx.strokeStyle='#f090a4'; ctx.lineWidth=2.5;
      ctx.beginPath();
      ctx.moveTo(bx,by);
      ctx.quadraticCurveTo((bx+ex)/2,(by+ey)/2+Math.sin(tk*0.5)*9,ex,ey);
      ctx.stroke();
      // Bout de la langue bien visible : on suit où elle va
      ctx.fillStyle='#ff8098';
      ctx.beginPath(); ctx.arc(ex,ey,4.5,0,6.28); ctx.fill();
      // Enroulée autour de la proie une fois atteinte
      if(t2>=1){
        ctx.strokeStyle='#e0607a'; ctx.lineWidth=3.5;
        ctx.beginPath(); ctx.ellipse(p.x,p.y,16,11,Math.sin(tk*0.2)*0.3,0,6.28); ctx.stroke();
      }
    }
    ctx.restore();
    return;
