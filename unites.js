...
const RACINES_SECONDAIRE = 0.6; // part des dégâts pour les ennemis pris dans les racines
const LIGNE_RACINES_LONGUEUR = 130;  // portée de la ligne de racines au sol
const LIGNE_RACINES_DUREE    = 150;  // 2,5 secondes avant que la ligne disparaisse
const LIGNE_RACINES_TICK     = 24;   // fréquence des dégâts pour qui reste dedans
...
plantSpirit:{hp:480,dmg:24,speed:0.55,range:42,color:'#4a7a3a'},
...
ancientTree:{hp:150,dmg:0,speed:0,range:0,color:'#6b4a2e'},
...
if(target){
 if(this.cool<=0){
  ...
  if(estTour){
   ...
   // fait apparaître une LIGNE de racines pointues devant lui. Une seule
   // ligne à la fois : un nouveau coup remplace l'ancienne au lieu de les
   // empiler indéfiniment tant qu'il combat.
   target.hp-=this.dmg*RALLY_DMG;
   const sens=this.team==='player'?1:-1;
   const i=racineZones.indexOf(this._zoneRacine);
   if(i>=0) racineZones.splice(i,1);
   this._zoneRacine={
...
const INFOS_UNITES = {
 ...
 plantSpirit: {nom:'Esprit des Plantes', pouvoir:'Colosse de bois et de racines. Chaque coup fait aussi surgir une LIGNE DE RACINES POINTUES au sol devant lui, qui blesse les ennemis qui restent dedans pendant environ 2,5 secondes avant de disparaître. À sa mort, un ARBRE ANCESTRAL pousse à sa place : il régénère tous les alliés dans une large zone, mais perd un peu de vie avec le temps et peut être détruit par l\'ennemi.'},
