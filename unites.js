...
if(this.type==='lizard'){
  ...
  if(this.phase==='chasse'){
    ...
    if(proie){
      // Trop près : il RECULE pour se remettre à bonne distance de tir.
      if(best<DISTANCE_TIR-20){
        const recul=this.team==='player'?-1:1;
        this.x += recul*this.speed*0.8;
        this.reculeur=true;
      } else this.reculeur=false;
      this.proieEnVue=true;

      // À bonne distance et rechargé : il harponne.
      if(this.recharge<=0 && best>=DISTANCE_TIR-20){
...
if(t==='necromancer'||t==='bannerman'||t==='skeleton'||t==='skeletonCard'
  ...
  if(t==='lizard'){
    ...
    // --- LA LANGUE, uniquement pendant l'attaque ---
    if(this.phase==='langue' && this.proie){
      ...
      ctx.beginPath();
      ctx.moveTo(bx,by);
      ctx.quadraticCurveTo((bx+ex)/2,(by+ey)/2+Math.sin(tk*0.4)*10,ex,ey);
      ctx.stroke();
      ctx.fillStyle='#ff8098';
      ctx.beginPath(); ctx.arc(ex,ey,5,0,6.28); ctx.fill();
      if(t2>=1){
        ctx.strokeStyle='#e0607a'; ctx.lineWidth=3.5;
        ctx.beginPath(); ctx.ellipse(p.x,p.y,16,11,0,0,6.28); ctx.stroke();
      }
    }
    ctx.restore();
    return;
