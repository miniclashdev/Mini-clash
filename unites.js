...
const LANGUE_PORTEE   = 190;   // jusqu'où la langue va chercher
const LANGUE_ALLER    = 14;    // images avant que la traction commence
const LANGUE_TRACTION = 4.2;   // vitesse à laquelle la proie est ramenée
...
if(this.type==='lizard'){
  ...
  if(this.phase==='chasse' && this.recharge<=0){
    ...
    if(proie){ this.phase='langue'; this.langueT=0; this.proie=proie; }
  }
  ...
  if(this.phase==='langue'){
    ...
    if(!p||p.hp<=0){ this.phase='chasse'; this.proie=null; }
      ...
      if(this.langueT>LANGUE_ALLER+90){   // la proie résiste trop longtemps
...
if(t==='necromancer'||t==='bannerman'||t==='skeleton'||t==='skeletonCard'
  ...
  if(t==='lizard'){
    ...
    if(this.phase==='langue' && this.proie){
      ...
      const t2=Math.min(1,this.langueT/LANGUE_ALLER);
      const ex=bx+(p.x-bx)*t2, ey=by+(p.y-by)*t2;
      // Elle ondule pendant qu'elle se déroule
      ctx.strokeStyle='#e0607a'; ctx.lineWidth=4; ctx.lineCap='round';
      ctx.beginPath();
      ctx.moveTo(bx,by);
      ctx.quadraticCurveTo((bx+ex)/2,(by+ey)/2+Math.sin(tk*0.5)*9,ex,ey);
      ctx.stroke();
      ctx.strokeStyle='#f090a4'; ctx.lineWidth=2;
      ctx.beginPath();
      ctx.moveTo(bx,by);
      ctx.quadraticCurveTo((bx+ex)/2,(by+ey)/2+Math.sin(tk*0.5)*9,ex,ey);
      ctx.stroke();
...
const INFOS_UNITES = {
 ...
 lizard:      {nom:'Lézard à Casque', pouvoir:'Projette sa LANGUE jusqu\'à 190 pour enrouler un ennemi et le ramener d\'un coup sur la pointe de son casque : 34 dégâts d\'empalement, puis il l\'achève à coups de griffes. Sa proie ne peut plus bouger pendant la traction. Les colosses de plus de 420 PV sont trop lourds pour être arrachés du sol.'},
