...
const RACINES_ZONE     = 65;    // rayon des pointes de racines à chaque coup
...
if(target){
 if(this.cool<=0){
  ...
  if(estTour){
   ...
   // Des pointes de racines jaillissent du sol autour de la cible : les
   // ennemis proches en prennent une part, comme un mini-trébuchet
   // enraciné plutôt que projeté.
   target.hp-=this.dmg*RALLY_DMG;
   for(let g=0;g<unitsArr.length;g++){
    const v=unitsArr[g];
    if(v===target||v.team===this.team||v.hp<=0||v.lane!==this.lane) continue;
    if(Math.hypot(v.x-target.x,(v.y-target.y)*0.8)<RACINES_ZONE) v.hp-=this.dmg*RALLY_DMG*RACINES_SECONDAIRE;
   }
